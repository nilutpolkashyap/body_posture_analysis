...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        # cap.set(cv2.CAP_PROP_FRAME_WIDTH, width/5)
        # cap.set(cv2.CAP_PROP_FRAME_HEIGHT, width/5)
        # Capture frames.
        success, image1 = cap.read()
        image = cv2.resize(image1, (width, height),fx=0,fy=0,interpolation= cv2.INTER_LINEAR)
        if not success:
            print("Null.Frames")
            break
        # Get fps.
